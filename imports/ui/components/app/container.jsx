...
import {
  withModalMounter,
  getModal,
} from "/imports/ui/components/common/modal/service";
    ...
    withTracker(({ intl, baseControls }) => {
      Users.find({ userId: Auth.userID, meetingId: Auth.meetingID }).observe({
        removed(userData) {
          ...
          setTimeout(() => {
            const queryCurrentUser = Users.find({
              userId: Auth.userID,
            ...
            if (queryCurrentUser.count() === 0) {
              if (userData.ejected) {
                ...
                const description =
                  Auth.isAuthenticating || Auth.loggedIn
      ...
      const { publishedPoll, voiceProp, randomlySelectedUser, layout } =
        currentMeeting;
      ...
      return {
        ...
        shouldShowPresentation:
          !shouldShowScreenshare && !shouldShowExternalVideo,
        ...
        hidePresentation: getFromUserSettings(
          "bbb_hide_presentation",
          LAYOUT_CONFIG.hidePresentation
        ),
        autoSwapLayout: getFromUserSettings(
          "bbb_auto_swap_layout",
          LAYOUT_CONFIG.autoSwapLayout
        ),
